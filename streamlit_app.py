...
if st.session_state.last_st!="" and st.session_state.last_st!=st_sig:
    ...
    if st.session_state.tg_token and st.session_state.tg_chat:
        send_tg(st.session_state.tg_token, st.session_state.tg_chat, auto_msg)
...
tab_chart, tab_profit, tab_alert, tab_tg = st.tabs(["📈 Chart", "💰 Profit", "🔔 ALERTS", "📲 TELEGRAM"])
...
with tab_chart:
    ...
    st.success("✅ Green toolbar + Grey box dono hata diye - Clean chart! Pinch zoom still ON")
...
st.caption("V37.3 CLEAN - No toolbar, No box select, Only pinch zoom + MACD + Supertrend")
