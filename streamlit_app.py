...
.header-glass{background: linear-gradient(135deg, rgba(255,255,255,0.08), rgba(255,255,255,0.03)); backdrop-filter: blur(25px); border: 1px solid rgba(255,255,255,0.12); border-radius: 24px; padding: 20px 24px; box-shadow: 0 8px 32px rgba(0,0,0,0.4);}
.top-premium{background: linear-gradient(135deg, rgba(0,209,255,0.15), rgba(112,0,255,0.15), rgba(0,255,136,0.08)); backdrop-filter: blur(20px); border: 1.5px solid rgba(0,209,255,0.25); border-radius: 24px; padding: 18px 22px; box-shadow: 0 10px 40px rgba(0,209,255,0.15);}
...
.stButton>button{background: linear-gradient(135deg, #00D1FF, #7000FF, #00FF88)!important; border: none!important; border-radius: 16px!important; color: white!important; font-weight: 900!important; height: 62px!important; letter-spacing: 1.5px!important; box-shadow: 0 8px 24px rgba(0,209,255,0.3)!important;}
.screener-buy{background: rgba(0,255,136,0.15); border: 1px solid #00FF88; border-radius: 12px; padding: 12px; margin: 6px 0;}
.screener-sell{background: rgba(255,77,106,0.15); border: 1px solid #FF4D6A; border-radius: 12px; padding: 12px; margin: 6px 0;}
</style>
...
with c1: user_input=st.text_input("search", value="CUPID", placeholder="Search CUPID, IOCL...", label_visibility="collapsed")
...
# AUTO TELEGRAM ALERT ON FLIP
if st.session_state.last_st!="" and st.session_state.last_st!=st_sig:
    msg=f"🚨 *{raw} FLIP!* {st.session_state.last_st} → *{st_sig}*\n💰 Live: Rs {round(live,2)}\n📈 {trend} | MACD {macd_sig}\n🎯 TGT {round(tgt,2)} SL {round(low_min,2)}"
    ...
    if st.session_state.boom:
        ...
        st.toast(f"🚨 {raw} {st_sig}!", icon="🔔")
st.session_state.last_st=st_sig

st.markdown(f"""<div class="top-premium"><div style="display:flex; justify-content:space-between; align-items:center;"><div><div style="display:flex; align-items:center; gap:12px;"><h2 style="color:white; margin:0; font-size:22px; font-family:Inter; font-weight:900;">{raw}</h2><span style="color:#8892b0; font-size:11px; font-family:JetBrains Mono; background:rgba(255,255,255,0.08); padding:4px 10px; border-radius:8px;">{ticker}</span><span style="background: {st_color}22; border:1px solid {st_color}; color:{st_color}; padding:5px 12px; border-radius:20px; font-size:10px; font-weight:800;">ST {st_sig}</span></div><p style="color:#8892b0; margin:8px 0 0 0; font-size:11px; font-family:JetBrains Mono;">LIVE <span style="color:white; font-weight:800;">{round(live,2)}</span> • TGT <span style="color:#00FF88;">{round(tgt,2)}</span> • SL <span style="color:#FF4D6A;">{round(low_min,2)}</span> • <span style="color:{sig_color};">{trend}</span></p></div><div style="text-align:right;"><p style="color:#8892b0; font-size:9px; margin:0; letter-spacing:2px; font-weight:700;">LIVE PRICE</p><p style="color:white; font-size:32px; font-weight:900; margin:0; font-family:JetBrains Mono;">Rs {round(live,2)}</p><div class="{sig_class}" style="margin-top:10px; display:inline-block; min-width:120px; text-align:center;">{sig}</div><p style="color:{st_color}; font-size:10px; margin:8px 0 0 0; font-weight:700; font-family:JetBrains Mono;">ST {st_sig} • MACD {macd_sig}</p></div></div></div>""", unsafe_allow_html=True)

tab_chart, tab_screen, tab_alert, tab_tg = st.tabs(["📈 Chart", "🔥 LIVE SCREENER", "🔔 ALERTS + BOOM", "📲 TELEGRAM"])
...
with tab_chart:
    ...
    fig=make_subplots(rows=3, cols=1, shared_xaxes=True, vertical_spacing=0.05, row_heights=[0.60,0.20,0.20], subplot_titles=(f"{raw} {trend} | ST {st_sig}", f"MACD {macd_sig}", "RSI"))
...
with tab_screen:
    st.markdown("### 🔥 Live Screener - Ek Click me Top Stocks")
    watchlist=["CUPID","IOCL","RELIANCE","TCS","INFY","SBIN","GAIL","ATGL","SUZLON","RVNL","IRFC","YESBANK","ZOMATO","PAYTM","ADANIPOWER"]
    if st.button("🚀 SCAN ALL NOW", use_container_width=True):
        ...
        for i, name in enumerate(watchlist):
            ...
            if not d.empty:
                c=d["Close"]; e20=c.ewm(20).mean(); e50=c.ewm(50).mean()
                last_p=float(c.iloc[-1]); live_p=get_live_price(t)
                if live_p==0: live_p=last_p
                _, s_dir=calc_st(d.tail(100))
                st_s="BUY" if s_dir.iloc[-1]==1 else "SELL"
                sig_s="BUY" if e20.iloc[-1]>e50.iloc[-1] and last_p>e20.iloc[-1] else "SELL" if e20.iloc[-1]<e50.iloc[-1] else "HOLD"
                results.append({"Stock":name, "Live":round(live_p,2), "Signal":sig_s, "ST":st_s})
            prog.progress((i+1)/len(watchlist))
        prog.empty()
        # show BUY first
        for r in results:
            if r["Signal"]=="BUY":
                st.markdown(f"""<div class="screener-buy"><div style="display:flex; justify-content:space-between;"><b style="color:white;">{r['Stock']}</b><span style="color:#00FF88; font-weight:900;">{r['Signal']} | ST {r['ST']}</span></div><p style="margin:4px 0 0 0; color:#8892b0; font-size:11px;">Rs {r['Live']}</p></div>""", unsafe_allow_html=True)
        for r in results:
            if r["Signal"]!="BUY":
                col="#FF4D6A" if r["Signal"]=="SELL" else "#FFAA00"
                st.markdown(f"""<div class="screener-sell"><div style="display:flex; justify-content:space-between;"><b style="color:white;">{r['Stock']}</b><span style="color:{col}; font-weight:900;">{r['Signal']} | ST {r['ST']}</span></div><p style="margin:4px 0 0 0; color:#8892b0; font-size:11px;">Rs {r['Live']}</p></div>""", unsafe_allow_html=True)
        st.success(f"Scanned {len(results)} stocks - {len([x for x in results if x['Signal']=='BUY'])} BUY found!")

with tab_alert:
    st.session_state.boom=st.toggle("🔊 BOOM Sound ON - Flip par beep", value=st.session_state.boom)
    st.session_state.live_mode=st.toggle("🟢 LIVE AUTO RELOAD + Telegram (30 sec)", value=st.session_state.live_mode)
    st.info("🔔 Supertrend BUY→SELL flip hote hi Telegram pe auto message + Boom sound ayega")
...
with tab_tg:
    ...
    with col1:
        if st.button("💾 Save", use_container_width=True):
            st.session_state.tg_token=tok; st.session_state.tg_chat=chat; st.success("✅ Saved! Ab flip par auto alert ayega")
    with col2:
        if st.button("🧪 Test Message", use_container_width=True):
            if tok and chat:
                ok=send_tg(tok, chat, f"✅ FinTrade Premium Test - {raw} LIVE Rs {round(live,2)} {sig} | ST {st_sig}")
                st.success("Sent!") if ok else st.error("Failed - Token/Chat ID check karo")
            else: st.error("Token/Chat ID dalo")
