# ============================================================================
# FinTrade V52 FINAL LOCKED 24H - DO NOT EDIT AFTER 08 SEP 2026
# BASE = YOUR V51 FINAL - V48.1 GREEN HIDE + NIFTY500 + PERFECT+NEWS+ULTIMATE
# 24H FIX = NEVER EMPTY + 24H LOCK - LOGIC 0% CHANGE - ONLY FALLBACK ADD
# ============================================================================
...
def score_stock(df, ai_prob):
    try:
        ...
        if ai_prob>=70: score+=15; reasons.append(f"AI {ai_prob} pct UP")
...
def get_morning_picks():
    ...
    all_scanned=[]; strict=[]; prog=st.progress(0); total=len(WATCHLIST)
    for idx,name in enumerate(WATCHLIST):
        ...
        if not df.empty and len(df)>50:
            ...
            item={"name":name,"score":sc,"reasons":rsns,"rsi":rsi,"adx":adx_v,"live":live,"target":tgt,"profit_pct":profit,"sl":sl,"atr_pct":atr,"ticker":t,"ai_prob":ai_prob,"ai_reason":ai_reason,"perfect_combo":perfect_combo,"perfect_reason":perfect_reason,"news_score":news_score,"news_reason":news_reason,"ultimate_combo":ultimate_combo,"ultimate_reason":ultimate_reason}
            all_scanned.append(item)
            if ai_prob>=60 and sc>=75 and 40<=rsi<=72:
                strict.append(item)
    prog.empty()
    final_picks = sorted(strict,key=lambda x:(x["ultimate_combo"],x["ai_prob"],x["score"]),reverse=True)[:2] if len(strict)>=2 else sorted(all_scanned,key=lambda x:(x["ultimate_combo"],x["ai_prob"],x["score"]),reverse=True)[:2]
