...
st.set_page_config(page_title="FinTrade V11 FIXED", layout="wide", page_icon="👑")
st.markdown('<h1 style="background: linear-gradient(90deg,#00D1FF,#FFD700);-webkit-background-clip:text;-webkit-text-fill-color:transparent;font-weight:900;">👑 FinTrade V11 ULTIMATE - FIXED</h1>', unsafe_allow_html=True)
st.caption("✅ TradingView PRO + PDF + WhatsApp + Voice - 1+2+3 FIXED")
...
st.divider()
tab1, tab2, tab3, tab4 = st.tabs(["TradingView PRO [1]", "PDF + WhatsApp [2]", "Voice + Intraday [3]", "Watchlist"])
...
if abs(last['Close']-res)/last['Close'] < 0.02:
    alert_msg = f"Alert! {clean} near resistance {res:.0f}"
elif abs(last['Close']-sup)/last['Close'] < 0.02:
    alert_msg = f"Alert! {clean} near support {sup:.0f}"
...
with tab2:
    ...
    with colB:
        share_text = f"FinTrade V11 Report: {ticker} LTP {last['Close']:.2f} S:{sup:.2f} R:{res:.2f} Trend {trend}"
...
with tab3:
    ...
    if voice_on:
        if st.button("Test Voice"):
            components.html(f"<script>var msg=new SpeechSynthesisUtterance('V11 active for {clean}');window.speechSynthesis.speak(msg);</script>", height=0)
