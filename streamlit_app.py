# ============================================================================
# V51 PERFECT + NEWS SENTIMENT - 08 SEP 2026 - 09:24 PM (Aapke Screenshot Time)
# OLD FEATURES - 100% LOCKED FROM V48.1 - QUESS + PAYTM DESIGN - NO DELETE
# NEW FEATURE - 7th AI NEWS SENTIMENT - ONLY ADD, NO DELETE
# ============================================================================
...
st.set_page_config(page_title="FinTrade V51 PERFECT+NEWS", layout="wide", page_icon="🔥")

# YOUR SCREENSHOT CSS - 100% SAME - NO DELETE
st.markdown("""
<style>
...
.pick-god{background: linear-gradient(135deg, rgba(0,255,136,0.10) 0%, rgba(0,209,255,0.08) 50%, rgba(112,0,255,0.08) 100%); backdrop-filter: blur(30px); border:1.5px solid rgba(0,255,136,0.25); border-radius: 24px; padding: 20px; overflow:hidden;}
...
.ai-badge{background: linear-gradient(135deg, #7000FF, #00D1FF); color:white; font-size:10px; padding:6px 14px; border-radius:100px; font-family:JetBrains Mono; font-weight:800; margin-left:6px;}
...
# ============================================================================
# OLD FEATURES 100% LOCKED - FROM V48.1 - DO NOT DELETE - YOUR SCREENSHOT
# ============================================================================
...
def get_perfect_combo(df, ai_prob_old):
    ...
    except: return ai_prob_old, "Combo Error"
# ============================================================================
# OLD END - 100% LOCKED - NO DELETE
# ============================================================================

# ============================================================================
# NEW ONLY ADD - 7th AI - NEWS SENTIMENT AI - NO DELETE OF OLD
# ============================================================================
...
def get_news_sentiment(ticker_symbol):
    try:
        ...
        positive = ["buy","upgrade","strong","growth","profit","bullish","gain","rise","high","record","beat"]
        negative = ["sell","downgrade","loss","bearish","fall","drop","low","miss","cut","warn"]
        pos = sum(1 for w in positive if w in titles)
        neg = sum(1 for w in negative if w in titles)
        if pos > neg: return 85, f"News Bullish +{pos}"
        elif neg > pos: return 25, f"News Bearish -{neg}"
        else: return 60, "News Neutral"
    except:
        return 50, "News N/A"

def get_ultimate_perfect_combo(df, ai_prob_old, perfect_old, news_score):
    try:
        # Old perfect + news = ultimate
        ultimate = int(perfect_old*0.75 + news_score*0.25)
        ultimate = min(99, max(0, ultimate))
        if ultimate>=90: r="7 AI STRONG BUY"
        elif ultimate>=80: r="6 AI BUY"
        elif ultimate>=70: r="5 AI BUY"
        else: r="WAIT"
        return ultimate, r
    except:
        return perfect_old, "6 AI"
...
def get_morning_picks():
    ist_now = datetime.now(pytz.timezone('Asia/Kolkata')); today=str(date.today()); current_slot="AM" if ist_now.hour<13 else "PM"
    if st.session_state.pick_date==today and st.session_state.pick_slot==current_slot and st.session_state.morning_picks: return st.session_state.morning_picks
    ...
    for idx,name in enumerate(WATCHLIST):
        ...
        if not df.empty and len(df)>50:
            ...
            if perfect_combo<75: continue
            # NEW ADD - NEWS SENTIMENT
            news_score, news_reason = get_news_sentiment(t)
            ultimate_combo, ultimate_reason = get_ultimate_perfect_combo(df, ai_prob, perfect_combo, news_score)
            if ultimate_combo<78: continue
    ...
    prog.empty()
    temp=sorted(temp,key=lambda x:(x["ultimate_combo"],x["perfect_combo"],x["ai_prob"]),reverse=True)[:2]
        ...
        with col:
            ...
            <div class="pick-god">
              ...
              <div class="perfect-row">
                ...
                <div style="font-family:JetBrains Mono; font-size:8px; font-weight:700; color:black; background: linear-gradient(135deg,#FFD700,#FF8C00); padding:4px 10px; border-radius:100px;">6 AI AGREE</div>
              </div>
              <div class="news-row">
