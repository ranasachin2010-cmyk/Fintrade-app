...
def get_logo():
    try:
        with open("logo.png","rb") as f: return f'<img src="data:image/png;base64,{base64.b64encode(f.read()).decode()}" width="60" style="border-radius:12px;">'
    except: return '<div style="font-size:40px;">💎</div>'

st.markdown(f"""<div class="header-box"><div style="display:flex; align-items:center; gap:16px;"><div>{get_logo()}</div><div><h1 style="margin:0; color:white; font-size:28px;">FinTrade Premium</h1><p style="margin:0; color:#8892b0; font-size:10px;">V37.2 CLEAN CHART - NO TOOLBAR | MACD + SUPERTREND + AUTO TELEGRAM</p></div></div></div>""", unsafe_allow_html=True)
...
def calc_macd(c, fast=12, slow=26, sig=9):
    ef=c.ewm(span=fast).mean(); es=c.ewm(span=slow).mean(); m=ef-es; s=m.ewm(span=sig).mean(); h=m-s
    return m,s,h
...
if df.empty:
    if "ADANI" in raw and "GAS" in raw: ticker="ATGL.NS"; df=load_data(ticker)
...
if st.session_state.last_st!="" and st.session_state.last_st!=st_sig:
    auto_msg=f"🚨 *{raw} AUTO FLIP* {st.session_state.last_st} -> {st_sig} LIVE {round(live,2)}"
    if st.session_state.tg_token and st.session_state.tg_chat: send_tg(st.session_state.tg_token, st.session_state.tg_chat, auto_msg)
    if st.session_state.boom: st.markdown('<audio autoplay><source src="https://actions.google.com/sounds/v1/alarms/beep_short.ogg" type="audio/ogg"></audio>', unsafe_allow_html=True)
st.session_state.last_st=st_sig

st.markdown(f"""<div class="top-pin"><div style="display:flex; justify-content:space-between;"><div><h2 style="color:white; margin:0; font-size:20px;">{raw} <span style="color:#8892b0; font-size:11px;">{ticker}</span> <span style="background:{sig_color}; color:black; padding:4px 12px; border-radius:20px; font-size:11px;">{sig}</span> <span style="background:{st_color}; color:black; padding:4px 10px; border-radius:20px; font-size:10px;">ST {st_sig}</span> <span style="background:rgba(0,209,255,0.3); color:#00D1FF; padding:4px 10px; border-radius:20px; font-size:10px;">MACD {macd_sig}</span></h2><p style="color:#00D1FF; margin:6px 0 0 0; font-size:11px;">LIVE {round(live,2)} | Target {round(tgt,2)} | SL {round(low_min,2)} | ST {round(st_line.iloc[-1],2)}</p></div><div style="text-align:right;"><p style="color:{sig_color}; font-size:26px; font-weight:900; margin:0;">Rs {round(live,2)}</p></div></div></div>""", unsafe_allow_html=True)

tab_chart, tab_profit, tab_alert, tab_tg = st.tabs(["📈 Chart CLEAN", "💰 Profit", "🔔 ALERTS + BOOM", "📲 TELEGRAM"])
...
with tab_chart:
    ...
    fig=make_subplots(rows=3, cols=1, shared_xaxes=True, vertical_spacing=0.05, row_heights=[0.60,0.20,0.20], subplot_titles=(f"{raw} {trend} | ST {st_sig} | {round(sup,2)}-{round(res,2)} | Pinch Zoom ON", f"MACD {macd_sig}", "RSI"))
    ...
    fig.add_trace(go.Scatter(x=df_c.index, y=st_line, line=dict(color=st_color,width=3), name=f"Supertrend"), row=1, col=1)
    ...
    # YAHAN SE TOOLBAR HATA - GREEN LINE WALA SAB GAYAB
    ...
    with c1: st.metric("TREND", trend, delta=sig)
    with c2: st.metric("SUPERTREND", st_sig, delta=f"{round(st_line.iloc[-1],2)}")
    with c3: st.metric("MACD", macd_sig, delta=f"{round(m_line.iloc[-1],3)}")
    with c4: st.metric("RSI", f"{round(rsi.iloc[-1],1)}")
    st.success("✅ Green toolbar hata diya - Chart clean! Pinch Zoom abhi bhi kaam karega 2 finger se")

with tab_profit:
    qty=st.number_input("Qty", min_value=1, value=100, step=1)
    buy_val=live*qty; target_val=tgt*qty; profit=target_val-buy_val
...
with tab_tg:
    ...
    if st.button("Save"): st.session_state.tg_token=tok; st.session_state.tg_chat=chat; st.success("Saved!")

st.caption("V37.2 CLEAN - Green toolbar removed - Pinch zoom still works")
